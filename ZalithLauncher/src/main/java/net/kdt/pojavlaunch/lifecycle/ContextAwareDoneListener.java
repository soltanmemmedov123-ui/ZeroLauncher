...
import com.movtery.zalithlauncher.feature.mod.conflict.ConflictReport;
import com.movtery.zalithlauncher.feature.mod.conflict.ModConflictDetector;
import com.movtery.zalithlauncher.feature.mod.conflict.ModConflictDialog;
...
public class ContextAwareDoneListener implements AsyncMinecraftDownloader.DoneListener, ContextExecutorTask {
            ...
            public void onParseEnded(@NonNull List<? extends ModInfo> modInfoList) {
                ...
                if (modInfoList.isEmpty()) executeTask();
                    ...
                    ContextExecutor.executeTaskWithAllContext(context -> new ModChecker().check(context, modInfoList, modCheckResult -> {
                        ...
                        runConflictDetectionAndLaunch(context, (List<ModInfo>) modInfoList);
                        return null;
                    }));
                }
            }
    ...
    /**
     * Runs the conflict detector off the UI thread, then either shows a dialog
     * (on the UI thread) or proceeds straight to launch if no issues exist.
     */
    private void runConflictDetectionAndLaunch(Context context, List<ModInfo> modInfoList) {
        // Respect the user's toggle for conflict checking
        if (!AllSettings.getBenchmarkEnableConflictCheck().getValue()) {
            executeTask();
            return;
        }
        TaskExecutors.getDefault().execute(() -> {
            ConflictReport report = ModConflictDetector.INSTANCE.detect(modInfoList);
            if (report.isEmpty()) {
                // Clean — launch immediately
                executeTask();
            } else {
                // Show dialog on UI thread
                TaskExecutors.runInUIThread(() -> {
                    if (context instanceof Activity) {
                        new ModConflictDialog(
                            context,
                            report,
                            mVersion,
