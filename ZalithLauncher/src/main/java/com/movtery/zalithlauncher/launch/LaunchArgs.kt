...
class LaunchArgs(
    ...
    private fun getLauncherJvmArgs(lwjglComponent: String): List<String> {
        ...
        // Inject ZeroAgent when developer tools are enabled
        val prefs = context.getSharedPreferences("zero_launcher_prefs", MODE_PRIVATE)
        if (prefs.getBoolean("enable_dev_tools", false) && LibPath.ZERO_AGENT.exists()) {
            args.add("-javaagent:${LibPath.ZERO_AGENT.absolutePath}")
        }
