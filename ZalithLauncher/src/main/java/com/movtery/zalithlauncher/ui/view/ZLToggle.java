...
public class ZLToggle extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float h = getHeight();
        float radius = h / 2f;
        float thumbRadius = dpToPx(7f);   // 14dp diameter thumb
        float margin = dpToPx(2f);        // 2dp margin from edges

        // Interpolate track color
        int trackColor = interpolateColor(COLOR_TRACK_OFF, COLOR_TRACK_ON_ALPHA, thumbPosition);
