...
class VersionsListFragment : FragmentWithAnim(R.layout.fragment_versions_list) {
    ...
    /** Index of the "Base" tab in favoritesFolderTab, or -1 when it isn't shown. */
    private var baseTabIndex: Int = -1
    /** The view for the "Base" tab so we can add/remove it cleanly. */
    private var baseTabView: View? = null
    ...
    private fun refreshFavoritesSelection(index: Int) {
        when {
            index == 0 -> refreshVersions(all = true)
            // "Base" tab is always the last tab; its index is stored in baseTabIndex
            baseTabIndex >= 0 && index == baseTabIndex -> refreshVersions(baseOnly = true)
            else -> {
                val folderNames = FavoritesVersionUtils.getFavoritesStructure().keys.toList()
                // Favorites start at index 1; base tab (if present) is after all favorites
                val folderName = folderNames.getOrNull(index - 1)
                refreshVersions(all = false, favoritesFolder = folderName)
            }
        }
    }
    ...
    private fun refreshVersions(
        all: Boolean = true,
        favoritesFolder: String? = null,
        baseOnly: Boolean = false
    ) {
        val adapter = versionsAdapter ?: return
        val allVersions = VersionsManager.getVersions()
        val baseNames = VersionsManager.getBaseVersionNames()

        val filteredVersions: List<Version> = when {
            // "Base" tab: show only the hidden vanilla dependency versions
            baseOnly -> allVersions.filter { it.getVersionName() in baseNames }

            // "All" tab: hide vanilla dependency versions (they live in the Base tab)
            all -> allVersions.filter { it.getVersionName() !in baseNames }
    ...
    private fun refreshFavoritesFolderAndVersions() {
        ...
        // Remove all dynamically added tabs (favorites + base tab)
        favoriteFolderViews.forEach { view ->
            binding.favoritesFolderTab.removeView(view)
        }
        favoriteFolderViews.clear()

        baseTabView?.let { binding.favoritesFolderTab.removeView(it) }
        baseTabView = null
        baseTabIndex = -1

        // Re-add favorites tabs (index 1..N)
        ...
        // If any vanilla base versions exist, append the "Base" tab after all favorites
        val baseNames = VersionsManager.getBaseVersionNames()
        if (baseNames.isNotEmpty()) {
            // Index = 1 (All) + favorites count + 1 would be one past the last; but DslTabLayout
            // is 0-based and we just need to know which tab index it lands on after addView.
            baseTabIndex = 1 + favoriteFolderViews.size  // "All" is 0, favorites are 1..N
            baseTabView = createBaseTabView().also {
                binding.favoritesFolderTab.addView(it)
            }
        }
    }

    private fun createBaseTabView(): View {
        return ItemFavoriteFolderBinding.inflate(layoutInflater).apply {
            name.text = getString(R.string.version_category_base)
            root.layoutParams = DslTabLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
            )
            // Long-press shows a small explanation tooltip
            root.setOnLongClickListener {
                Toast.makeText(
                    requireContext(),
                    getString(R.string.version_category_base_tip),
                    Toast.LENGTH_LONG
                ).show()
                true
            }
