...
object VersionsManager {
    ...
    /**
     * Returns the set of version names that are "vanilla base" installs —
     * pure Minecraft versions (no mod loader) that were downloaded automatically
     * as a dependency for at least one modloaded version (Forge, Fabric, etc.).
     *
     * Logic:
     *  1. Collect the minecraftVersion of every version that HAS a mod loader.
     *  2. Any vanilla version (loaderInfo null/empty) whose folder name appears in that
     *     set is a dependency base — the user didn't explicitly install it.
     */
    fun getBaseVersionNames(): Set<String> {
        val allVersions = versions.toList()

        // MC version strings declared by modloaded versions as their base
        val modpackBaseMcVersions: Set<String> = allVersions
            .filter { it.getVersionInfo()?.loaderInfo?.isNotEmpty() == true }
            .mapNotNull { it.getVersionInfo()?.minecraftVersion }
            .toHashSet()

        return allVersions
            .filter { version ->
                val info = version.getVersionInfo() ?: return@filter false
                info.loaderInfo.isNullOrEmpty() &&                         // no loader = vanilla
                        version.getVersionName() in modpackBaseMcVersions  // referenced by a modpack
            }
