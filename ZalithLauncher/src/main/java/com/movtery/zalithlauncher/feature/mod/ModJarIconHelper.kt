...
object ModJarIconHelper {
    ...
    // Max size we ever display mod icons at, in pixels.
    // 48dp * 4x density = 192px – use inSampleSize so we never decode a 512x512 into RAM.
    private const val ICON_MAX_PX = 192

    private fun decodeSampledBitmap(input: java.io.InputStream): android.graphics.Bitmap? {
        // Two-pass decode: first read bounds only, then pick inSampleSize, then decode pixels.
        val bytes = input.readBytes()
        val opts = BitmapFactory.Options().apply { inJustDecodeBounds = true }
        BitmapFactory.decodeByteArray(bytes, 0, bytes.size, opts)
        var sample = 1
        while ((opts.outWidth / sample) > ICON_MAX_PX || (opts.outHeight / sample) > ICON_MAX_PX) {
            sample *= 2
        }
    ...
    private fun loadDrawableFromZip(context: Context, zip: ZipFile, path: String): Drawable? {
        ...
        zip.getInputStream(entry).use { input ->
            val bitmap = decodeSampledBitmap(input) ?: return null
