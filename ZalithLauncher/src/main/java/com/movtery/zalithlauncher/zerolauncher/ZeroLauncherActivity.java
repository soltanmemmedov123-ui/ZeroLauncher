...
public class ZeroLauncherActivity extends AppCompatActivity {
    ...
    // Pre-parsed nav tint colors – avoids Color.parseColor() on every page switch
    private static final int COLOR_NAV_ACTIVE   = 0xFF38BDF8;
    private static final int COLOR_NAV_INACTIVE = 0xFF9A9AB0;
    private static final ColorStateList CSL_NAV_ACTIVE   = ColorStateList.valueOf(COLOR_NAV_ACTIVE);
    private static final ColorStateList CSL_NAV_INACTIVE = ColorStateList.valueOf(COLOR_NAV_INACTIVE);

    // Cached interpolator – reused across all page transitions instead of allocating each time
    private static final DecelerateInterpolator PAGE_INTERPOLATOR = new DecelerateInterpolator();
    ...
    // Dev-tools button (in icon_nav, visibility controlled by enable_dev_tools pref)
    private FrameLayout devToolsNavButton;

    // Dev-tools toggle (in page_settings)
    private ZLToggle toggleDevTools;

    // Pref key for developer tools visibility
    private static final String KEY_DEV_TOOLS = "enable_dev_tools";

    // Cached page-out AnimatorListenerAdapter – allocated once, reused on every transition
    // outPage is set just before each use so the listener always references the right view
    private View pendingOutPage;
    private final AnimatorListenerAdapter pageOutListener = new AnimatorListenerAdapter() {
        @Override public void onAnimationEnd(Animator animation) {
            if (pendingOutPage != null) {
                pendingOutPage.setVisibility(View.GONE);
                pendingOutPage.setAlpha(1f);
            }
        }
    };

    // Cached portrait-overlay reference (looked up once, not on every config change)
    private View portraitOverlay;

    // Cached memory bar update Runnable – avoids allocating a new Runnable every drag frame
    private int pendingMemoryIdx;
    private int pendingMemoryTotal;
    private final Runnable memBarUpdateRunnable = () -> {
        if (memoryBarFill == null) return;
        ViewGroup parent = (ViewGroup) memoryBarFill.getParent();
        if (parent == null) return;
        int parentW = parent.getWidth();
        float fraction = (float) pendingMemoryIdx / (pendingMemoryTotal - 1);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        portraitOverlay = findViewById(R.id.portrait_overlay);
    ...
    private void initNavigation() {
        ...
        // Dev-tools button — opens DevToolsActivity, not a launcher page
        devToolsNavButton = findViewById(R.id.nav_dev_tools);
        if (devToolsNavButton != null) {
            devToolsNavButton.setOnClickListener(v -> {
    ...
    private void switchToPage(int pageIdx, boolean animate) {
        ...
        // Update nav state – use pre-parsed ColorStateLists, no Color.parseColor() per call
        for (int i = 0; i < navButtons.length; i++) {
            boolean active = (i == pageIdx);
            navButtons[i].setSelected(active);
            navIndicators[i].setVisibility(active ? View.VISIBLE : View.GONE);
            navIcons[i].setImageTintList(active ? CSL_NAV_ACTIVE : CSL_NAV_INACTIVE);
        }
        ...
        if (animate) {
            ...
            inPage.animate()
                ...
                .translationX(0f)
                .setDuration(180)
                .setInterpolator(PAGE_INTERPOLATOR)   // cached – no allocation
                .start();

            // Use the cached listener; set the target view just before starting
            pendingOutPage = outPage;
            outPage.animate()
                .alpha(0f)
                .setDuration(120)
                .setListener(pageOutListener)         // cached – no allocation
                .start();
        } else {
        ...
        // NOTE: page index is persisted in onStop() – no need to write on every tap
    }
    ...
    private void setupSlider(SeekBar slider, TextView label, String key,
        ...
        slider.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override public void onProgressChanged(SeekBar s, int progress, boolean fromUser) {
                // Update label on every frame for live feedback – no disk I/O here
                label.setText(fmt.format(progress + min));
            }
            @Override public void onStartTrackingTouch(SeekBar s) {}
            @Override public void onStopTrackingTouch(SeekBar s) {
                // Write to prefs only once, when the finger lifts – was firing every drag pixel
                prefs.edit().putInt(key, s.getProgress() + min).apply();
            }
        });
    }
    ...
    private void initJavaPage() {
        ...
        if (sliderMemory != null) {
            ...
            sliderMemory.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                @Override public void onProgressChanged(SeekBar s, int progress, boolean fromUser) {
                    // Live label + bar update – no disk I/O on every frame
                    updateMemoryDisplay(memSteps[progress], memSteps.length, progress);
                }
                @Override public void onStartTrackingTouch(SeekBar s) {}
                @Override public void onStopTrackingTouch(SeekBar s) {
                    // Single write when finger lifts
                    prefs.edit().putInt("memory_idx", s.getProgress()).apply();
                }
            });
        }
    ...
    private void updateMemoryDisplay(int mb, int total, int idx) {
        ...
        if (memoryBarFill != null) {
            // Use cached Runnable; store parameters in fields so the lambda-free Runnable can read them
            pendingMemoryIdx   = idx;
            pendingMemoryTotal = total;
            memoryBarFill.removeCallbacks(memBarUpdateRunnable); // drop any pending frame
            memoryBarFill.post(memBarUpdateRunnable);
        }
    }
    ...
    private void launchGame() {
        ...
        try {
            ...
            Class<?> launchActivity = Class.forName(
                "net.kdt.pojavlaunch.LauncherActivity");
    ...
    private void checkOrientation() {
        if (portraitOverlay == null) return;
        int orientation = getResources().getConfiguration().orientation;
        portraitOverlay.setVisibility(
    ...
    protected void onResume() {
        super.onResume();
        // Re-evaluate every time (user may have changed pref externally or via DevToolsActivity)
        updateDevToolsButtonVisibility();
    }
